        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <!-- Header -->
                    <div class="mb-6">
                        <h2 class="text-2xl font-bold text-gray-900">Edit Request #{{ $request->id }}</h2>
                        <p class="text-gray-600">Update your pickup request details</p>
                    </div>

                    <!-- Form -->
                    <form action="{{ route('user.requests.update', $request) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                                ...
                                <select name="device_condition" id="device_condition" required
                                    ...
                                    @foreach ($deviceConditions as $condition)
                                        <option value="{{ $condition }}"
                                            {{ old('device_condition', $request->device_condition) == $condition ? 'selected' : '' }}>
                                            {{ ucfirst($condition) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea name="pickup_address" id="pickup_address" rows="4" required
                                    ...
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('pickup_address', $request->pickup_address) }}</textarea>
                            ...
                            <div>
                                ...
                                <input type="date" name="preferred_pickup_date" id="preferred_pickup_date"
                                    ...
                                    value="{{ old('preferred_pickup_date', $request->preferred_pickup_date ? $request->preferred_pickup_date->format('Y-m-d') : '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                            ...
                            <div>
                                ...
                                <textarea name="user_note" id="user_note" rows="3"
                                    ...
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('user_note', $request->user_note) }}</textarea>
                                @error('user_note')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
